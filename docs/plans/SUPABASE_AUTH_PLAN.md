  ...
  - Configure project URL + publishable key for the desktop app.
  ...
  - Create a `frontend/src/services/supabaseClient.ts`:
    - Initialize client with URL + publishable key.
    - Provide `signInWithPassword`, `signOut`, `getSession`, `refreshSession`.
  - Add environment configuration (Vite) for Supabase URL + publishable key.
