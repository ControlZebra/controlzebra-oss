 ...
 */
const BranchItem = memo(function BranchItem({ branch, isCurrent, onSelect }) {
  return (
    <button
...
function BranchModal({ open, onClose }) {
  ...
  const [mode, setMode] = useState('switch'); // 'switch' or 'create'
  ...
  const handleSwitch = useCallback(async (branchName) => {
