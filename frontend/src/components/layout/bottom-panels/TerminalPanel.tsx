...
interface ExtendedTerminal extends Terminal {
  _eventCancels?: Array<(() => void) | undefined>;
}
...
function TerminalPanel(): JSX.Element {
  ...
  const initTerminal = useCallback(async (): Promise<void> => {
    if (!terminalRef.current || xtermRef.current) return;

    // Create xterm instance
    const term: ExtendedTerminal = new Terminal({
      cursorBlink: true,
      fontSize: 13,
    ...
    const outputCancel = Events.On('terminal-output:' + result.sessionId, (event: { data?: string }) => {
    ...
    // Listen for errors
    const errorCancel = Events.On('terminal-error:' + result.sessionId, (event: { data?: string }) => {
      if (xtermRef.current && event.data) {
    ...
    term._eventCancels = [outputCancel, errorCancel, exitCancel];
  }, [repoPath]);
