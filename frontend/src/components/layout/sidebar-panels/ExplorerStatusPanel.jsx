 ...
 */
function PanelLayout({ type, title, subtitle, children }) {
  ...
  return (
    <div className="p-4 text-center">
      <div className={`inline-flex items-center justify-center w-12 h-12 rounded-full ${iconBg} mb-3`}>
        <Icon style={ICON_STYLES.lg} className={iconColor} />
      </div>
 ...
 * 
 * Props:
 * - status: 'noRepo' | 'synced' | 'push' | 'featureBranch'
 * - folderName: string (for noRepo status)
 * - branchName: string (for featureBranch status)
 * - ahead: number (commits ahead, for push status)
 * - hasUpstream: boolean (for push status)
 * - totalLocalCommits: number (for push status when no upstream)
 * - onInitialize: () => void (for noRepo)
 * - onSync: () => void (for push)
 * - isLoading: boolean
 * - isSyncing: boolean
 */
