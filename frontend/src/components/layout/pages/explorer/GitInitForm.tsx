 ...
 */
import { memo, useState, useCallback, useMemo, useEffect, type CSSProperties, type ChangeEvent, type KeyboardEvent, type FormEvent, type MouseEvent } from 'react';
import {
  Download,
  FolderPlus,
  X,
...
interface LFSAttribute {
  pattern: string;
  description: string;
}

interface LFSPlatform {
  platform: string;
  extensions: string[];
  description: string;
}

interface LFSCategory {
  label: string;
  icon: LucideIcon;
  color: string;
  bgColor: string;
  platforms: LFSPlatform[];
}
...
interface GitInitFormProps {
  folderName: string;
  onBack: () => void;
  onSubmit?: (data: GitInitData) => void;
  isLoading?: boolean;
}
...
interface LFSAttributeInputProps {
  attributes: LFSAttribute[];
  onAdd: (attr: LFSAttribute) => void;
  onRemove: (index: number) => void;
  suggestions?: LFSAttribute[];
}

interface ModeSelectionProps {
  onSelectMode: (mode: 'clone' | 'init') => void;
  onBack: () => void;
}
...
const iconStyleLg: CSSProperties = { width: 48, height: 48 };
...
// ============================================================================
const LFS_EXTENSION_GROUPS: Record<string, LFSCategory> = {
...
const COMMON_LFS_PATTERNS: LFSAttribute[] = Object.values(LFS_EXTENSION_GROUPS).flatMap(category =>
...
const LFSAttributeInput = memo(function LFSAttributeInput({ 
...
}: LFSAttributeInputProps): JSX.Element {
  const [inputValue, setInputValue] = useState('');
  const [showDropdown, setShowDropdown] = useState(false);
  const [expandedCategories, setExpandedCategories] = useState<Record<string, boolean>>({});
  const [activeTab, setActiveTab] = useState<'groups' | 'custom' | 'search'>('groups');
  const [customGroups, setCustomGroups] = useState<CustomLFSGroup[]>([]);
  ...
  const loadCustomGroups = useCallback(async (): Promise<void> => {
    try {
  ...
  const toggleCategory = useCallback((categoryKey: string): void => {
  ...
  const handleAddPlatformGroup = useCallback((platform: LFSPlatform): void => {
  ...
  const handleAddCustomGroup = useCallback((group: CustomLFSGroup): void => {
    group.extensions.forEach(ext => {
      const pattern = `*${ext}`;
      if (!addedPatterns.has(pattern)) {
  ...
  const handleAddExtension = useCallback((ext: string, platformName: string): void => {
    const pattern = `*${ext}`;
    if (!addedPatterns.has(pattern)) {
  ...
  const isPlatformFullyAdded = useCallback((platform: LFSPlatform): boolean => {
    return platform.extensions.every(ext => addedPatterns.has(`*${ext}`));
  }, [addedPatterns]);

  // Check if a platform is partially added
  const isPlatformPartiallyAdded = useCallback((platform: LFSPlatform): boolean => {
  ...
  const isCustomGroupFullyAdded = useCallback((group: CustomLFSGroup): boolean => {
    return group.extensions.every(ext => addedPatterns.has(`*${ext}`));
  }, [addedPatterns]);

  // Check if a custom group is partially added
  const isCustomGroupPartiallyAdded = useCallback((group: CustomLFSGroup): boolean => {
  ...
  const handleSuggestionClick = useCallback((suggestion: LFSAttribute): void => {
    onAdd(suggestion);
    setInputValue('');
  }, [onAdd]);
...
// ============================================================================
const ModeSelection = memo(function ModeSelection({ onSelectMode, onBack }: ModeSelectionProps): JSX.Element {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          <div className="w-12 h-12 rounded-full bg-blue-500/10 flex items-center justify-center mb-4 group-hover:bg-blue-500/20 transition-colors">
            <Download style={{ ...iconStyleLg, width: 24, height: 24 }} className="text-blue-500" />
          </div>
        ...
        >
          <div className="w-12 h-12 rounded-full bg-green-500/10 flex items-center justify-center mb-4 group-hover:bg-green-500/20 transition-colors">
            <FolderPlus style={{ ...iconStyleLg, width: 24, height: 24 }} className="text-green-500" />
          </div>
...
// ============================================================================
const CloneForm = memo(function CloneForm({ onBack, onSubmit, isLoading = false }: CloneFormProps): JSX.Element {
  ...
  const [lfsAttributes, setLfsAttributes] = useState<LFSAttribute[]>([]);
  const [showAdvanced, setShowAdvanced] = useState(false);

  const handleChange = useCallback((field: string) => (e: ChangeEvent<HTMLInputElement>): void => {
    const value = e.target.type === 'checkbox' ? e.target.checked : e.target.value;
    setFormData(prev => ({ ...prev, [field]: value }));
  }, []);

  const handleAddLfsAttribute = useCallback((attr: LFSAttribute): void => {
    setLfsAttributes(prev => [...prev, attr]);
  }, []);

  const handleRemoveLfsAttribute = useCallback((index: number): void => {
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <label className="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
              checked={lfsEnabled}
              onChange={(e: ChangeEvent<HTMLInputElement>) => setLfsEnabled(e.target.checked)}
...
// ============================================================================
const InitForm = memo(function InitForm({ onBack, onSubmit, isLoading = false, folderName }: InitFormProps): JSX.Element {
  const [formData, setFormData] = useState({
    userName: '',
    userEmail: '',
    initialBranch: 'main',
...
// ============================================================================
function GitInitForm({ folderName, onBack, onSubmit, isLoading = false }: GitInitFormProps): JSX.Element {
  const [mode, setMode] = useState<'clone' | 'init' | null>(null);
  ...
  const handleSubmit = useCallback((data: GitInitData): void => {
