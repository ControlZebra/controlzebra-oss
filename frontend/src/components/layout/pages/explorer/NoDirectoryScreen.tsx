 ...
 */
import { memo, useState, useEffect, useCallback, type CSSProperties, type KeyboardEvent, type MouseEvent } from 'react';
...
interface NoDirectoryScreenProps {
  onOpenFolder: () => void;
...
function NoDirectoryScreen({ onOpenFolder, onOpenPath, isLoading = false }: NoDirectoryScreenProps): JSX.Element {
  ...
  const handleRemoveRecent = useCallback((e: MouseEvent<HTMLSpanElement> | KeyboardEvent<HTMLSpanElement>, path: string): void => {
    e.stopPropagation();
        ...
        {recentFolders.length > 0 && (
          <div className="mt-8 text-left">
            <div className="flex items-center gap-2 text-theme-muted text-xs uppercase tracking-wide mb-3">
              <Clock style={ICON_STYLES.xs as CSSProperties} />
              <span>Recent</span>
            </div>
            <ul className="space-y-1">
              {recentFolders.slice(0, MAX_RECENT_DISPLAY).map((path) => (
                <li key={path}>
                  ...
                  >
                    <Folder style={ICON_STYLES.sm as CSSProperties} className="text-yellow-500 shrink-0" />
