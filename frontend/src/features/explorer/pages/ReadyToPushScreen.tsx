...
interface ReadyToPushScreenProps {
  ahead?: number;
  hasUpstream?: boolean;
...
function ReadyToPushScreen({ 
  ...
  onSync, 
  isSyncing = false,
}: ReadyToPushScreenProps): JSX.Element {
    ...
    <div className="flex-1 flex items-center justify-center p-8 animate-screen-enter overflow-y-auto">
      <div className="max-w-lg w-full text-center">
        ...
        <div className="inline-flex items-center justify-center w-16 h-16 rounded-full bg-blue-500/10 mb-6">
          <Save style={ICON_STYLES.xl as CSSProperties} className="text-blue-400" />
        </div>
        ...
        >
          {hasUpstream ? (
            <Cloud style={ICON_STYLES.md as CSSProperties} />
          ) : (
            <Upload style={ICON_STYLES.md as CSSProperties} />
          )}
